...
const db = {
     ...
     */
    async init() {
        if (helpers && typeof helpers.showLoading === 'function') {
            helpers.showLoading('正在加载云端数据...');
        }
        this._data = {
            admins: [],
            teachers: [],
        ...
        try {
            ...
            if (helpers && typeof helpers.hideLoading === 'function') {
                helpers.hideLoading();
            }
        }
    ...
    async syncToCloud() {
        try {
            ...
            if (helpers && typeof helpers.showToast === 'function') {
                helpers.showToast(`同步失败：${e.message || '未知错误'}`, 'warning');
            }
        }
    },
